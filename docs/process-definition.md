...
изменения payload и наоборот. Runtime передает `payloadSchemaVersion` в `ProcessPayloadMapper`:
default Jackson mapper требует совпадения сохраненной schema version с definition, а кастомный mapper
может поддержать старые schema versions или upcasters.
Для одного `processType + payloadSchemaVersion` registry требует один Java payload type; разные
`definitionVersion` могут переиспользовать одну payload schema, если тип payload совпадает.
...
Для `StepResult.retryableFailure(...)` можно задать retry policy. Пока retry budget не исчерпан,
runtime сохраняет retry metadata и планирует delayed `RETRY` command. Когда budget исчерпан,
runtime по умолчанию продолжает обычный выбор transition по `StepResult`; если задан
`retryExhaustedTargetState`, он сразу переводит процесс в это состояние с trigger type
`RETRY_EXHAUSTED`.
...
Паркует процесс на заданную задержку и после нее автоматически переводит instance в целевое
состояние. Это состояние подходит для polling-сценариев, когда внешний сервис принял команду
асинхронно, но финальный результат надо узнавать периодическим REST-запросом.

TIMER задается через:

- `delay` - через сколько продолжить процесс;
- `targetState` - состояние, куда runtime перейдет после задержки.

При входе в TIMER runtime сохраняет `state_deadline_at`, планирует delayed resume command и
переводит instance в статус `WAITING`. При срабатывании timer в history пишется trigger `TIMER`.
...
- `timerState("STATE", state -> state.delay(...).targetState(...))`;
- `transition(t -> t.name(...).targetState(...).condition(...))`.
- `retryExhaustedTargetState(...)` - target для `ACTION`, если retryable failure исчерпал
...
- у `TIMER` должны быть `delay` и `targetState`, но не должно быть transitions;
- у `TERMINAL` должен быть terminal status и не должно быть transitions;
- `retryExhaustedTargetState` можно задавать только у `ACTION`;
- у non-terminal states должны быть исходящие transitions;
- transition, timeout и retry exhausted targets должны указывать на существующие states;
...
Для тестов process definitions доступен `process-manager-testkit`: structural assertions,
deterministic runner без PostgreSQL и fake command scheduler.
...
runner.signal("payment.result", "payment-1", Map.of("status", "APPROVED"));
...
Polling после `202 Accepted` описывается как явный timer loop:
            ...
            .transition(
                transition ->
                    transition
                        .name("pending")
                        .targetState("WAIT_NEXT_POLL")
                        .condition(ctx -> ctx.resultCodeEquals("PENDING")))
            .otherwise("TECHNICAL_FAILURE"))
```
